...
public class RoadConeZombie : Monster, IDamageable
{
    ...
    public void Update()
    {
        ...
        if (stateMachine.CurrentKey != StateKey.Die)
        {
            ...
            if (Data.Health <= 0)
            {
                ...
                else if (Data is CommonZombieData)//本体生命值为0,死亡
                {
    ...
    class Walk : MonsterState //魔物的走路状态
    {
        ...
        public Walk(RoadConeZombie target)
        {
    ...
    class Attack : MonsterState //魔物的攻击状态
    {
        private Rigidbody2D rigid;
        private Animator animator;
        private RoadConeZombie target;
        private Coroutine attackCoroutine;
        /// <summary>
        /// 受到攻击的目标
        /// </summary>
        public IDamageReceiver ReceiverUnderAttack { get; set; }
        public Attack(RoadConeZombie target)
        {
            this.target = target;
            rigid = target.GetComponent<Rigidbody2D>();
            animator = target.GetComponent<Animator>();
    ...
    class Stun : MonsterState //魔物的眩晕状态
    {
        ...
        public Stun(RoadConeZombie target)
        {
            rigid = target.GetComponent<Rigidbody2D>();
            animator = target.GetComponent<Animator>();
        }
    ...
    {
        private RoadConeZombie target;
        public Die(RoadConeZombie target)
        {
            this.target = target;
        }
        public override void OnEnterState()
