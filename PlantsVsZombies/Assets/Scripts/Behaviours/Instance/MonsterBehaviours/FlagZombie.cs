...
/// 摇旗子僵尸的行动脚本
/// </summary>
public class FlagZombie : Monster, IDamageable
{
    /// <summary>
    ...
    public void Update()
    {
        ...
        if (stateMachine.CurrentKey != StateKey.Die)
        {
            if (Data.Health <= 0)
            {
                StateKey before = stateMachine.CurrentKey;

                stateMachine.TriggerAction("Die");
                
                switch (before) //保持原来的姿态
                {
                    case StateKey.Walk:
    ...
    class Attack : MonsterState //魔物的攻击状态
    {
        private Rigidbody2D rigid;
        private Animator animator;
        private FlagZombie target;
        private Coroutine attackCoroutine;
        /// <summary>
        /// 受到攻击的目标
        /// </summary>
        public IDamageReceiver ReceiverUnderAttack { get; set; }
        public Attack(FlagZombie target)
        {
            this.target = target;
            rigid = target.GetComponent<Rigidbody2D>();
            animator = target.GetComponent<Animator>();
    ...
    class Die : MonsterState //魔物的死亡状态
    {
        ...
        public override void OnEnterState()
        {
            ...
            target.FallingHead.transform.SetParent(MonstersController.MonsterFatherObject.transform);
